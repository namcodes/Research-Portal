                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            ...
                            $folder_query = $conn->query("SELECT * FROM tbl_category WHERE folder_name = 'Student' ORDER BY _id ASC");
                            $file = '';
                            if ($folder_query->num_rows > 0) {
                                foreach ($folder_query as $tbl_r) {
                                    $file_query = $conn->query("SELECT c.category_id as category_id, c.name as category, f.file_name as file_name, c.slog as slog, f.file_description as file_desc FROM tbl_files f LEFT JOIN tbl_category c ON f.category_id = c.category_id WHERE c.folder_name = 'Student' AND f.category_id = '{$tbl_r['category_id']}'");

                                    foreach ($file_query as $file_info) {
                                        $file .= '
                                        <a href="view.php?v=' . $file_info['file_name'] . '&f=' . $file_info['file_desc'] . '&mode=students" data-id="1" class="btn_view d-block mb-2 px-2 col-md-4">
